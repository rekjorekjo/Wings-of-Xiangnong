package com.wings.module.product.dal.mysql.storeproductreply;
...
public interface StoreProductReplyMapper extends BaseMapperX<StoreProductReplyDO> {
    ...
    @Select("select A.product_score as productScore,A.service_score as serviceScore," +
            ...
            " from wings_store_product_reply A left join wings_user B " +
            "on A.uid = B.id left join wings_store_order_cart_info C on A.`unique` = C.`unique`" +
