package com.ordering.module.product.dal.mysql.storeproductreply;
...
public interface StoreProductReplyMapper extends BaseMapperX<StoreProductReplyDO> {
    ...
    @Select("select A.product_score as productScore,A.service_score as serviceScore," +
            ...
            " from app_store_product_reply A left join app_user B " +
            "on A.uid = B.id left join app_store_order_cart_info C on A.`unique` = C.`unique`" +
