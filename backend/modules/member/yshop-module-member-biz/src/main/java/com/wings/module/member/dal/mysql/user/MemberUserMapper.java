package com.wings.module.member.dal.mysql.user;
...
import com.wings.module.member.controller.admin.user.vo.UserExportReqVO;
import com.wings.module.member.controller.admin.user.vo.UserPageReqVO;
...
public interface MemberUserMapper extends BaseMapperX<MemberUserDO> {
    ...
    @Update("update wings_user set now_money=now_money-#{payPrice}" +
            " where id=#{uid}")
    int decPrice(@Param("payPrice") BigDecimal payPrice, @Param("uid") Long uid);

    @Update("update wings_user set integral=integral-#{score}" +
            " where id=#{uid}")
    int decScore(@Param("score") Integer score, @Param("uid") Long uid);

    @Update("update wings_user set pay_count=pay_count+1" +
            " where id=#{uid}")
    int incPayCount(@Param("uid") Long uid);

    @Update("update wings_user set now_money=now_money+#{price}" +
            " where id=#{uid}")
