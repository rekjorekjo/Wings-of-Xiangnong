package com.ordering.module.message.supply;
...
import com.ordering.module.member.api.user.dto.WechatUserDto;
...
import com.ordering.module.message.service.wechattemplate.WechatTemplateService;
...
public class WeiXinSubscribeService {
    ...
    public void rechargeSuccessNotice(String time,String price,Long uid){
        ...
        map.put("remark", ShopConstants.APP_WECHAT_PUSH_REMARK);
    ...
    private String getTempId(String key){
        WechatTemplateDO legacyWechatTemplate = wechatTemplateService.lambdaQuery()
                .eq(WechatTemplateDO::getType,"subscribe")
                .eq(WechatTemplateDO::getTempkey,key)
                .one();
        if (legacyWechatTemplate == null) {
            throw exception(new ErrorCode(9999999,"请后台配置key:" + key + "订阅消息id"));
        }
        if(ShopCommonEnum.IS_STATUS_0.getValue().equals(legacyWechatTemplate.getStatus())){
            return "";
        }
        return legacyWechatTemplate.getTempid();
    }
    ...
    private String getUserOpenid(Long uid){
        MemberUserDO legacyUser = userService.getById(uid);
        if(legacyUser == null) {
            return "";
        }
        if(StrUtil.isBlank(legacyUser.getRoutineOpenid())) {
            return "";
        }
        return legacyUser.getRoutineOpenid();
