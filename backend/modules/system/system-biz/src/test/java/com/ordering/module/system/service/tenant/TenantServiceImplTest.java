package com.ordering.module.system.service.tenant;

import com.ordering.framework.common.enums.CommonStatusEnum;
import com.ordering.framework.common.pojo.PageResult;
import com.ordering.framework.tenant.config.TenantProperties;
import com.ordering.framework.tenant.core.context.TenantContextHolder;
import com.ordering.framework.test.core.ut.BaseDbUnitTest;
import com.ordering.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
import com.ordering.module.system.controller.admin.tenant.vo.tenant.TenantSaveReqVO;
import com.ordering.module.system.dal.dataobject.permission.MenuDO;
import com.ordering.module.system.dal.dataobject.permission.RoleDO;
import com.ordering.module.system.dal.dataobject.tenant.TenantDO;
import com.ordering.module.system.dal.dataobject.tenant.TenantPackageDO;
import com.ordering.module.system.dal.mysql.tenant.TenantMapper;
import com.ordering.module.system.enums.permission.RoleCodeEnum;
import com.ordering.module.system.enums.permission.RoleTypeEnum;
import com.ordering.module.system.service.permission.MenuService;
import com.ordering.module.system.service.permission.PermissionService;
import com.ordering.module.system.service.permission.RoleService;
import com.ordering.module.system.service.tenant.handler.TenantInfoHandler;
import com.ordering.module.system.service.tenant.handler.TenantMenuHandler;
import com.ordering.module.system.service.user.AdminUserService;
...
import static com.ordering.framework.common.util.collection.SetUtils.asSet;
import static com.ordering.framework.common.util.date.LocalDateTimeUtils.buildBetweenTime;
import static com.ordering.framework.common.util.date.LocalDateTimeUtils.buildTime;
import static com.ordering.framework.common.util.object.ObjectUtils.cloneIgnoreId;
import static com.ordering.framework.test.core.util.AssertUtils.assertPojoEquals;
import static com.ordering.framework.test.core.util.AssertUtils.assertServiceException;
import static com.ordering.framework.test.core.util.RandomUtils.*;
import static com.ordering.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
import static com.ordering.module.system.enums.ErrorCodeConstants.*;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("app", user.getUsername());
            assertEquals("yuanma", user.getPassword());
            assertEquals("app", user.getNickname());
        ...
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setContactName("app");
            o.setContactMobile("15601691300");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setWebsite("http://localhost:80");
            o.setUsername("app");
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("app"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("app");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    public void testGetTenantByWebsite() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setWebsite("http://localhost:80"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByWebsite("http://localhost:80");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
