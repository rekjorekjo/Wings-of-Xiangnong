...
import com.wings.framework.test.core.util.RandomUtils;
...
import com.wings.module.system.mq.message.mail.MailSendMessage;
import com.wings.module.system.mq.producer.mail.MailProducer;
...
//                .setFrom("奥特曼 <ydym_test@163.com>")
    ...
    public void testDoSendMail_success() {
        try (final MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            // 准备参数
            MailSendMessage message = randomPojo(MailSendMessage.class, o -> o.setNickname("wings"));
            // mock 方法（获得邮箱账号）
            ...
            mailUtilMock.when(() -> MailUtil.send(
                    argThat(mailAccount -> {
                        assertEquals("wings <7685@qq.com>", mailAccount.getFrom());
