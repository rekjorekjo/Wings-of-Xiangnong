package com.wings.module.system.service.tenant;

import com.wings.framework.common.enums.CommonStatusEnum;
import com.wings.framework.common.pojo.PageResult;
import com.wings.framework.tenant.config.TenantProperties;
import com.wings.framework.tenant.core.context.TenantContextHolder;
import com.wings.framework.test.core.ut.BaseDbUnitTest;
import com.wings.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
import com.wings.module.system.controller.admin.tenant.vo.tenant.TenantSaveReqVO;
import com.wings.module.system.dal.dataobject.permission.MenuDO;
import com.wings.module.system.dal.dataobject.permission.RoleDO;
import com.wings.module.system.dal.dataobject.tenant.TenantDO;
import com.wings.module.system.dal.dataobject.tenant.TenantPackageDO;
import com.wings.module.system.dal.mysql.tenant.TenantMapper;
import com.wings.module.system.enums.permission.RoleCodeEnum;
import com.wings.module.system.enums.permission.RoleTypeEnum;
import com.wings.module.system.service.permission.MenuService;
import com.wings.module.system.service.permission.PermissionService;
import com.wings.module.system.service.permission.RoleService;
import com.wings.module.system.service.tenant.handler.TenantInfoHandler;
import com.wings.module.system.service.tenant.handler.TenantMenuHandler;
import com.wings.module.system.service.user.AdminUserService;
...
import static com.wings.framework.common.util.collection.SetUtils.asSet;
import static com.wings.framework.common.util.date.LocalDateTimeUtils.buildBetweenTime;
import static com.wings.framework.common.util.date.LocalDateTimeUtils.buildTime;
import static com.wings.framework.common.util.object.ObjectUtils.cloneIgnoreId;
import static com.wings.framework.test.core.util.AssertUtils.assertPojoEquals;
import static com.wings.framework.test.core.util.AssertUtils.assertServiceException;
import static com.wings.framework.test.core.util.RandomUtils.*;
import static com.wings.module.system.dal.dataobject.tenant.TenantDO.PACKAGE_ID_SYSTEM;
import static com.wings.module.system.enums.ErrorCodeConstants.*;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            assertEquals("wings", user.getUsername());
            assertEquals("yuanma", user.getPassword());
            assertEquals("wings", user.getNickname());
        ...
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setContactName("wings");
            o.setContactMobile("15601691300");
            o.setPackageId(100L);
            o.setStatus(randomCommonStatus());
            o.setWebsite("https://www.wings.co");
            o.setUsername("wings");
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("wings"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("wings");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
    ...
    public void testGetTenantByWebsite() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setWebsite("https://www.wings.co"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByWebsite("https://www.wings.co");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
